...
| `.gitignore` + `.context/PBI/` (cache-tier POLICY only) | Whether `.context/PBI/` is committed or gitignored, and the `[SYNC]`/`[COMMIT]`/`[LOCAL]` tier model itself, is repo cache architecture, not ticket content. NOT the CONTENT inside `.context/PBI/` — that stays `/sprint-testing`'s (see FORBIDDEN row below, now scoped to content). |
...
| `.context/PBI/` (CONTENT — story/epic/bug `.md` bodies) | `/sprint-testing` — per-ticket QA context. `framework-development` may touch `.gitignore` and cache-tier POLICY (see ALLOWED row above) but never hand-authors or edits a PBI content file. |

...
**Jira Key:** [BK-448](https://upexgalaxy71.atlassian.net/browse/BK-448)
**Status:** Candidate
...
| `{own_workspace_slug}` | The authenticated member's own workspace, distinct from the target story's workspace |
| `{target_story_id}` | A real story ID belonging to a different workspace (row 1), or a random UUID with no matching row (row 2) |
